"""
Experiment: Low-Rank Connectivity
"""
...
def train_lowrank(data_path, output_dir, rank=10, max_epochs=500, patience=100, device='cpu', seed=42):
    ...
    model = create_model_from_data_extended(
        ...
        n_inputs=dataset.get_input_dim(), model_type='low_rank', rank=rank,
    ...
    for epoch in tqdm(range(max_epochs), desc=f"rank={rank}"):
        model.train()
    ...
    with open(output_dir / 'results.json', 'w') as f:
        json.dump({'config': {'rank': rank}, 'best_val_corr': best_val_corr, 'history': history}, f, indent=2)
...
def main():
    ...
    base_dir = Path('results/lowrank_experiments')
    base_dir.mkdir(parents=True, exist_ok=True)
    
    experiments = [{'name': f'rank_{r}', 'rank': r} for r in [5, 10, 20, 50]]
    results = []
    ...
    for exp in experiments:
        name = exp.pop('name')
        print(f"\n{name}")
        corr = train_lowrank(data_path=data_path, output_dir=str(base_dir / name), **exp)
